...
version: 1.1.0
category: brand
description: "Construieste si rafineaza identitatea vizuala a unui brand din orice material de referinta (PDF-uri, URL-uri, screenshot-uri, postari existente, brand docs) — incremental. Produce DOUA output-uri sincronizate: (1) artefacte machine-readable in brand/visual-identity/ (tokens.json, identity.md, moves.md, fonts/, logos/, compositions/) consumate de skill-urile vizuale downstream; (2) un brand bible visual-identity.pdf (deck 23-layout). Ruleaza si un Template Factory mode care genereaza variatii de template-uri HTML per format (ex. 3 hero + 10 body + 3 cta pentru linkedin-carousel). Foundation skill — ruleaza inainte de orice skill de executie care randeaza vizualuri (ssc-designer, ssc-image-generator, viz-image-gen). 5 moduri: Extract, Import, Build, Auto-Scrape, Templates."
triggers:
  - "identitate vizuala"
  - "visual identity"
  - "brand identity"
  - "design tokens"
  - "construieste identitate vizuala"
  - "build brand identity"
  - "extrage stilul vizual"
  - "extract visual"
  - "replica stilul asta"
  - "replicate this style"
  - "match this design"
  - "genereaza template-uri din pdf"
  - "generate templates from PDF"
  - "creeaza brand bible"
  - "create brand bible"
  - "brand design"
negative_triggers:
  ...
  - "genereaza o imagine"
context_loads:
  - context/learnings.md (section brand-visual-identity)
inputs:
  - mode (optional: extract | import | build | auto-scrape | templates — auto-detectat)
  - source (optional: PDF, URL, screenshots, brand doc)
  - format (optional: pentru Templates mode, ex. linkedin-carousel)
outputs:
  - brand/visual-identity/ (tokens.json, identity.md, moves.md, fonts/, logos/, compositions/)
  - brand/visual-identity/visual-identity.pdf (brand bible)
  - brand/templates/<pool>/ (HTML templates + manifest.json + preview-uri)
secrets_optional:
  - FIRECRAWL_API_KEY
  - OPENAI_API_KEY
runtime_dependencies:
  - "python: >=3.11"
  - "pip: scikit-image, fonttools, pillow, numpy, pyyaml, requests, pymupdf, jinja2"
...
- User wants written voice extraction → `brand-voice`
- User wants positioning/angle → `brand-positioning`
- User wants ICP/audience → `brand-audience`
...
- **Single brand per project.** One `brand/` per project root. No multi-tenant model yet.
...
**Invocation context:** when this skill is invoked by a parent orchestrator (e.g. `00-social-content` Phase 1 brand-context guard), do NOT print a "welcome" or "to start, run /brand-visual-identity" message. Jump straight to the guided intake — the parent already confirmed the user wants to configure visual identity.
...
- **Brand asset** — owned by the user's brand. Logo, headshot, existing posts they made. Goes to `brand/visual-identity/{logos|headshots|posts-archive}/`. NOT a composition reference; the renderer treats these as content to be EMBEDDED.
- **Template reference** — inspiration the user wants to copy. Other brands, magazine spreads, screenshots from accounts they admire. Goes to `brand/visual_refs/`. The composition is EXTRACTED via Phase 4.5 (vision pass).
...
|---|---|
| BRAND (logo) | `brand/visual-identity/logos/<slug>.png` + run `intake_logo.py` for bg cleanup |
| BRAND (headshot) | `brand/visual-identity/headshots/<slug>.jpg` |
| BRAND (own post / archive) | `brand/visual-identity/posts-archive/<slug>.png` (NOT extracted; informs voice/style only) |
| TEMPLATE-REF (default for everything unchecked) | `brand/visual_refs/<slug>.png` → Phase 4.5 extraction |
...
| primary = X, non-primary = "sibling" | Creates `{decoupled_base}/../{sibling-slug}/brand/` as a parallel project. Warn the user of the path and proceed with primary only in this run |
...
5. **Token consolidation** — merge per-reference analyses into single `brand/visual-identity/tokens.json` + `identity.md` + `moves.md`. **User-locked fields** (set manually by the user) are preserved verbatim.
     ...
     questions: [{
       ...
       options: [
         { label: "Yes, seed neutral-identity",
           description: "Copies defaults/neutral-identity to brand/visual-identity/ with your brand name. You can refine it later." },
...
**Input:**
- `brand/visual-identity/tokens.json` (must exist — run Mode E/I/B first)
- `brand/visual-identity/moves.md` (optional but recommended)
- `output_format` name — **`linkedin-carousel` only in v1** (other formats spec'd but deferred)

**Output:**
- `brand/templates/{output_format}/`
...
1. **Verify foundation** — confirm `brand/visual-identity/tokens.json` exists. If not, route user to Mode E/I/B first.
...
3. **Generate variation HTMLs** — for each role (hero/body/cta) generate N templates by varying:
   ...
   - Brand moves from `brand/visual-identity/moves.md` applied (paper-texture, accent-bar, oversized-numeral, etc.)
   - All using the SAME type scale and palette from `brand/visual-identity/tokens.json` (consistency across variations)
...
**Universal rule:** every phase that mutates `brand/` MUST print at start:
...
3. Consolidate everything into `brand/visual-identity/tokens.json` + `identity.md` + `moves.md`
```
...
- For Google Fonts pairings, download to `{brand_context}/visual-identity/fonts/` via `fetch_font.py` (stdlib-only Google Fonts CSS2 resolver, no API key). Emits one `.woff2` per weight/style (`latin` subset; covers PT-BR/ES accents). Example: `uv run skills/brand-visual-identity/scripts/fetch_font.py --family "Inter Tight" --weights 900 --family "Inter" --weights 400,500,700 --output-dir brand/visual-identity/fonts`.
...
```bash
uv run --with playwright --with pillow python skills/brand-visual-identity/scripts/generate_brand_bible_pdf.py \
    --brand-context brand/ \
    --output brand/visual-identity/brand-book.pdf
```
...
Once you've decided the families, write the YAML directly to `brand/visual-identity/template-families.yaml` via the Write tool.
...
Read `brand/visual-identity/template-families.yaml` (Phase 4.95 output) and filter to families where `has_image_zone: true`. If NONE of the families have it, SKIP this phase (the brand has no AI image requirement; templates are pure text + brand chrome).

If `brand/visual-identity/ai-image-style.md` already exists with `status: validated`, ALSO SKIP — the brand contract is locked. Re-run only when the user explicitly asks.
...
uv run skills/viz-image-gen/scripts/generate_image_gpt.py \
...
```
[ok] brand/visual-identity/ai-image-style.md  (status: first-encounter)
[sample] brand/visual-identity/_ai-style-test/sample.png — review before Phase 5
```
...
families = yaml_load("brand/visual-identity/template-families.yaml")["families"]
...
for family in families:
    ...
    template_dir = f"brand/templates/{pool}/{slug}/"

    result = Agent({
      subagent_type: "ssc-template-builder",
      description: f"Template build — {slug}",
      prompt: f"""You are the ssc-template-builder agent (your system prompt is already loaded — do NOT re-read it).
...
```bash
uv run skills/brand-visual-identity/scripts/validate_brand.py \
    --brand-context brand/
```
...
If `template-families.yaml` has a `styles:` section (≥2 styles approved in Phase 4.97), write the consumer-facing `brand/templates/{pool}/styles.json` now that every family is built and `status: ready`. The shape + role rules are specified inline below (this phase is self-contained); the file's carousel-time consumer is `00-social-content`'s `ssc-designer`, when that pack is installed.
...
═══════════════════════════════════════════════════════════
  ...
  Templates:    {N_ready} ready / {N_rejected} rejected
                 ({list_slugs})
  Brand bible:  brand/visual-identity/brand-book.pdf
...
**Hard rule for downstream skills (00-social-content, ssc-designer, ssc-image-generator):** they MUST check `brand/templates/{pool}/manifest.json` exists AND has ≥1 entry with `status: "ready"` BEFORE running content-inference / content-gen phases. If not, refuse to proceed.
...
- On BUILD or BOTH → invoke `brand-voice` + `brand-visual-identity` in parallel with the user-provided material.
- This skill scans for `brand/visual-identity/tokens.json`. If missing, runs Mode E (or Mode B if no refs).

**Consumed by:**
- `ssc-designer` reads `brand/visual-identity/moves.md` to apply the brand's design moves to each slide.
- `ssc-image-generator` reads `brand/visual-identity/tokens.json` + `brand/templates/{output_format}/*.html` to render slides. The renderer (`viz-image-gen/scripts/render_template.py`) resolves `brand/templates/{pool}/` first, with `viz-image-gen/references/templates/{pool}/` as fallback.
...
1. **Confirm `{brand_context}/` exists** at project root (created by `brand-voice` typically). If not, create it.
...
4. **Walk reference categories — one prompt per category, in this order:**
   ...
   **On upload — single atomic command (copy + safe bg removal):**
   ...
   ```bash
   uv run skills/brand-visual-identity/scripts/intake_logo.py \
   ...
   2. Copies to `brand/visual-identity/logos/`
